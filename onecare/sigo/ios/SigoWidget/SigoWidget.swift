...
struct Provider: TimelineProvider {
    ...
    func getSnapshot(in context: Context, completion: @escaping (SimpleEntry) -> ()) {
        ...
        let entry = SimpleEntry(
            ...
            resolved: data?.integer(forKey: "ticket_resolved") ?? 0,
            ...
            VStack(alignment: .leading, spacing: 6) {
                ...
                HStack(spacing: 12) {
                    ...
                    VStack(alignment: .leading, spacing: 4) {
                        ...
                        LegendItem(color: Color(red: 0.61, green: 0.15, blue: 0.69), label: NSLocalizedString("widget_resolved", comment: "Resolved"), count: entry.resolved)
                    }
                    .font(.system(size: 9))
                }
...
struct DonutChartView: View {
    ...
    var total: Int {
        acknowledged + held + inProgress + pending + resolved
    }
    ...
    var body: some View {
        ZStack {
            if total > 0 {
                ...
                DonutSegment(
                    startAngle: -90 + (360.0 * Double(acknowledged + held + inProgress + pending) / Double(total)),
                    amount: Double(resolved),
                    total: Double(total),
