...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    // Safe NotificationService access
        ...
        child: BlocListener<AuthBloc, AuthState>(
          ...
          listener: (context, state) {
            if (state is AuthUnauthenticated &&
              ...
              if (l10nContext != null) {
                final message = AppLocalizations.of(l10nContext).sessionExpired;
          ...
          child: BlocBuilder<LocaleCubit, Locale>(
            builder: (context, locale) => _AppLifecycleObserver(
              child: MaterialApp(
                title: 'SIGO OneCare',
              ...
              theme: AppTheme.light().copyWith(
                ...
                routes: {
                  ...
                  AppRoutes.home: (context) => const ConfigWrapper(),
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.resumed && mounted) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted) {
          context.read<LocaleCubit>().syncWithSystem();
        }
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder<void>(
      ...
      builder: (context, snapshot) {
        ...
        return StreamBuilder<TenantConfig?>(
          stream: configRepository.configStream,
          initialData: configRepository.currentConfig,
...
class _AuthWrapperState extends State<AuthWrapper> {
  ...
  void _startInitialLoads(BuildContext context) {
    if (_didTriggerInitialLoad) return;
    _didTriggerInitialLoad = true;
    final ticketBloc = context.read<TicketBloc>();
    ticketBloc.add(const ClearTickets());
    unawaited(_runInitialSyncTasks(context, ticketBloc));
  }

  Future<void> _runInitialSyncTasks(
    ...
    final tasks = <Future<void> Function()>[
      ...
      () async {
        ...
        await ticketBloc.stream.firstWhere(
          (state) => state.hasLoadedOnce || (!state.isLoading && state.error != null),
        );
      },
    ];
    ...
    Future<void> worker() async {
      while (true) {
        ...
        try {
          ...
          debugPrint('AuthWrapper: Initial task failed: $e');
        }
      }
    }
    ...
    return BlocConsumer<AuthBloc, AuthState>(
      ...
      listener: (context, state) {
        ...
        if (state is AuthAuthenticated) {
          ...
          context.read<TicketBloc>().add(const ClearTickets());
          if (AppFeatures.enableNotifications) {
            getIt<NotificationService>().unbindAuthService();
          }
          _didUpdateWidgetStats = false;
          _didTriggerInitialLoad = false;
          if (state.reason == LogoutReason.configReset) {
            final navigator = rootNavigatorKey.currentState;
            if (navigator != null) {
              // Navigate directly to the QR scanner after config reset
      ...
      builder: (context, state) {
        ...
        if (state is AuthAuthenticated) {
          ...
          final ticketBloc = context.read<TicketBloc>();
          if (!_didTriggerInitialLoad &&
              !ticketBloc.state.hasLoadedOnce &&
              !ticketBloc.state.isLoading) {
            WidgetsBinding.instance.addPostFrameCallback((_) {
              if (!mounted) return;
              _startInitialLoads(context);
            });
          ...
          if (!_didUpdateWidgetStats) {
            ...
            WidgetsBinding.instance.addPostFrameCallback((_) {
              ...
              if (getIt.isRegistered<HomeWidgetService>()) {
                ...
                debugPrint('HomeWidgetService not yet registered, skipping widget stats update');
              }
            });
          }
