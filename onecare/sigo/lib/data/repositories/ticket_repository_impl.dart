...
class TicketRepositoryImpl implements TicketRepository {
  ...
  Future<void> _loadTickets({Map<String, dynamic>? query}) async {
    if (_isLoading) {
      debugPrint('Load tickets skipped: already loading');
      return;
    }
    ...
    try {
      final response = await _apiService.searchTickets(
        ...
        onTimeout: () {
          throw Exception('Request timeout: Unable to load tickets');
        },
      );

      final results = response['results'] as List;
      _totalCount = _extractTotalCount(response) ?? results.length;
  ...
  Future<void> _loadMoreTickets() async {
    ...
    try {
      ...
      final response = await _apiService.searchTickets(
        ...
        const Duration(seconds: 30),
        onTimeout: () {
          throw Exception('Request timeout: Unable to load more tickets');
        },
      );
      ...
      if (totalCount != null) {
        ...
        _totalCount ??= _tickets.length + results.length;
      }
      // Parse tickets in background isolate to prevent UI blocking
  ...
  int? _extractTotalCount(Map<String, dynamic> response) {
    ...
    if (paging is Map<String, dynamic>) {
      ...
      if (pagingTotal is num) {
        return pagingTotal.toInt();
      }
      if (pagingTotal is String) {
        final parsed = int.tryParse(pagingTotal);
        if (parsed != null) {
          return parsed;
        }
      }
