...
class TenantConfig {
  ...
  factory TenantConfig.fromQrCode(String qrData) {
    ...
    if (!json.containsKey('tenant')) {
      throw const FormatException('Invalid QR code format: missing "tenant" field');
    }
    if (!json.containsKey('iam_url')) {
      throw const FormatException('Invalid QR code format: missing "iam_url" field');
    }

    // Validate required onecare fields
    if (!onecareData.containsKey('baseurl')) {
      throw const FormatException('Invalid QR code format: missing "onecare.baseurl" field');
    }
    if (!onecareData.containsKey('service_id')) {
      throw const FormatException('Invalid QR code format: missing "onecare.service_id" field');
    }
