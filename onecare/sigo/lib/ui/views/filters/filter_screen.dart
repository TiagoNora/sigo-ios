...
class _FilterScreenState extends State<FilterScreen>
  ...
  void initState() {
    ...
    _setActiveFilter(
      widget.initialQuery ?? state.filterQuery,
      sourceId: widget.initialSourceFilterId ?? state.filterSourceId,
      markHydrated: true,
    );
    _loadSavedFilters();
    _loadCatalogs();
    ...
    if (end.isBefore(start)) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(l10n.endDateMustBeAfterStartDate),
  ...
  Future<void> _loadSavedFilters() async {
    ...
    try {
      ...
      if (_activeFilterQuery != null) {
        _setActiveFilter(
          _activeFilterQuery,
          sourceId: _activeFilterSourceId,
          populate: true,
        );
      }
    } catch (e) {
  ...
  void _applyQuickFilters() {
    ...
    _setActiveFilter(query, sourceId: null, populate: true);
  ...
  bool _hasFilterChanges() {
    ...
    final activeNormalized = _asJsonQuery(_activeFilterQuery);
    if (activeNormalized == null) return true;

    // Use normalized comparison to ignore ordering/noise
    return !_queriesEqual(query, activeNormalized);
  }
  ...
  Future<void> _saveQuickFilter() async {
    ...
    if (query == null) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text(l10n.noFiltersSelected)));
      return;
    }

    // Determine if we have a selected private filter
    SavedFilter? selectedPrivateFilter;
    if (_activeFilterSourceId != null) {
      try {
        final source = _savedFilters.firstWhere(
          (f) => f.id == _activeFilterSourceId,
        );
        if (source.visibility == 'PRIVATE') {
          selectedPrivateFilter = source;
        }
      } catch (e) {
        debugPrint('Error finding private filter: $e');
      }
    }

    if (selectedPrivateFilter == null && _activeFilterQuery != null) {
      try {
        selectedPrivateFilter = _savedFilters.firstWhere(
          (f) =>
              f.visibility == 'PRIVATE' &&
              _queriesEqual(f.config.toJson(), _activeFilterQuery),
        );
      } catch (e) {
        debugPrint('No matching private filter found: $e');
      }
    }

    final nameController = TextEditingController(
      text: selectedPrivateFilter?.name ?? '',
    );
              ...
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        l10n.ifYouKeepCurrentNameFilterWillBeUpdated,
    ...
    if (result == true && nameController.text.isNotEmpty) {
      try {
        ...
        final shouldUpdate =
            selectedPrivateFilter != null &&
            nameController.text == selectedPrivateFilter.name;
        ...
        if (shouldUpdate) {
          ...
          filterData['id'] = selectedPrivateFilter.id;
          await apiService.updateFilter(filterData);

          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(content: Text(l10n.filterUpdatedSuccessfully)),
            );
          }
        } else {
          // Create new filter
          await apiService.createFilter(filterData);

          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(content: Text(l10n.filterSavedSuccessfully)),
            );
          }
        }
        ...
        if (mounted) {
          ScaffoldMessenger.of(
            context,
          ).showSnackBar(SnackBar(content: Text('Failed to save filter: $e')));
        }
      }
    }
  }
  ...
  Future<void> _deleteSavedFilter(SavedFilter filter) async {
    ...
    if (confirmed == true) {
      try {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text(l10n.filterDeletedSuccessfully)),
          );
          _loadSavedFilters();
        }
      } catch (e) {
  ...
  bool _hasDraftActiveFilter() {
    if (_activeFilterQuery == null) return false;
    if (_activeFilterSourceId != null) return false;

    final source = _findDraftSourceFilter();
    if (source != null &&
        _queriesEqual(source.config.toJson(), _activeFilterQuery)) {
      return false;
    }

    return !_savedFilters.any(
      (filter) => _queriesEqual(filter.config.toJson(), _activeFilterQuery),
    );
  }

  SavedFilter? _findDraftSourceFilter() {
    if (_activeFilterSourceId == null) return null;
    try {
      return _savedFilters.firstWhere((f) => f.id == _activeFilterSourceId);
    } catch (e) {
  ...
  Widget _buildSavedFiltersTab() {
    ...
    return RefreshIndicator(
      ...
      child: ListView.builder(
        padding: const EdgeInsets.all(16),
        itemCount: _savedFilters.length + (hasDraft ? 1 : 0),
        itemBuilder: (context, index) {
          ...
          final filter = _savedFilters[filterIndex];
          final isActive = _isFilterActive(filter);
          final isDraftSource =
              hasDraft && draftSource != null && draftSource.id == filter.id;
                        ...
                        child: Builder(
                          builder: (context) {
                            ...
                            return OutlinedButton(
                              onPressed: hasChanges ? _saveQuickFilter : null,
