...
class TicketCard extends StatelessWidget {
  ...
  // Cache DateFormat instances to avoid recreating them on every build
  static final _timeFormat = DateFormat.Hm();
  static final _monthDayFormat = DateFormat.MMMd();

  // Cache PriorityRepository to avoid getIt calls on every build
  static final _priorityRepository = getIt<PriorityRepository>();

  @override
  Widget build(BuildContext context) {
    final l10n = AppLocalizations.of(context);
    final priority = _priorityRepository.getPriorityByName(
      ticket.priority.toString(),
    );
    final priorityColor = priority != null
        ? _priorityRepository.parseColor(priority.color)
        : Colors.grey;
            ...
            children: [
              ...
              Row(
                children: [
                  ...
                  const SizedBox(width: 4),
                  Text(
                    _formatDate(ticket.createdAt, l10n),
                    style: Theme.of(context).textTheme.bodySmall?.copyWith(
                      color: Colors.grey.shade600,
                    ),
                  ),
